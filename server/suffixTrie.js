...
class SuffixTrie {
     ...
     * Если более точно time complexity: n*(m + m*(m + m))
     * при n >> m получим сложность 2*n*m*m => O(n*m*m)
     * какая сложность у String.prototype.substring?github webkit
     * (здесь выше предполагаю, что это m)
     */
    _build() {
     ...
     * O(m), где m длинна искомой строки s
     * @param {String} s
     * @return {String[]}
     */
    find(s) {
